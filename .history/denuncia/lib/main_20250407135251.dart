    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(
          ...
          surface: Colors.white, // 👈 Color de fondo del AppBar
        ),
        textTheme: TextTheme(
...
class guardadoExitoso extends StatefulWidget {
  const guardadoExitoso({super.key});

  @override
  _guardadoExitosoState createState() => _guardadoExitosoState();
}

class _guardadoExitosoState extends State<guardadoExitoso> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("EXITOSAMENTE SE GUARDO"),
      ...
      body: Container(
        ...
        child: SingleChildScrollView(
          ...
          child: Padding(
            ...
            child: Text(
              "guardado exitosamente pantalla",
              style: TextStyle(
                fontSize: 16,
...
class FormDenunciaState extends State<FormDenuncia> {
  ...
  Future<void> _guardarDenuncia() async {
    if (_formkey.currentState!.validate()) {
      try {
        await DatabaseHelper().insertDenuncia(_crearDenuncia());
        _mostrarAlerta('Éxito', 'Denuncia registrada correctamente');
      } catch (e) {
        _mostrarAlerta('Error', 'Error al guardar la denuncia: $e');
      }
    }
  }

  // 4. Método para mostrar alertas
                  ...
                  try {
                    ...
                    setState(() {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => guardadoExitoso(),
                        ),
                      );
                    });

                    // Navegar de regreso después de guardar
                    Navigator.pop(context);

                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Denuncia guardada exitosamente!'),
