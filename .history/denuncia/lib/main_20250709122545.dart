...
class guardadoExitoso extends StatelessWidget {
          ...
          children: [
            ...
            _buildCard(
              FutureBuilder<List<Evidencia>>(
                future: DatabaseHelper().getEvidenciasPorDenuncia(denuncia.id),
                builder: (context, snapshot) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      ...evidencias.map(
                        (evidencia) => _buildEvidenciaItem(evidencia),
                      ),

                      const SizedBox(height: 8),
                      if (denuncia.servidorEvidencia.isNotEmpty)
                        FileUploadSection(
                          tipoEvidencia: denuncia.servidorEvidencia,
                          onUrlsObtenidas: (urls) {
                            // Manejar nuevas URLs de evidencias
                          },
                          onError: (error) {
  ...
  Widget _buildEvidenciaItem(Evidencia evidencia) {
    final esImagen =
        evidencia.url.toLowerCase().endsWith('.jpg') ||
        evidencia.url.toLowerCase().endsWith('.jpeg') ||
        evidencia.url.toLowerCase().endsWith('.png');
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                InkWell(
                  onTap: () async {
                    if (await canLaunchUrl(Uri.parse(evidencia.url))) {
                      await launchUrl(Uri.parse(evidencia.url));
                    }
                  },
                  child: Text(
...
class FormDenunciaState extends State<FormDenuncia> {
  ...
  final GlobalKey<FileUploadSectionState> _uploaderKey = GlobalKey();
              ...
              onPressed: () async {
                ...
                if (_formkey.currentState!.validate() && !hasErrors) {
                  try {
                    ...
                    print('Denuncia guardada exitosamente con ID: $idDenuncia');
                    ...
                    print('URLs de evidencias: $_urlsEvidencias');
