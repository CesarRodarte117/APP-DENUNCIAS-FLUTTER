              ...
              onPressed: () async {
                ...
                if (!archivosSubidos) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Error al subir archivos')),
                  );
                  return;
                }

                // 2. Guardas la denuncia principal y obtienes su ID
                final denunciaId =
                    await _guardarDenuncia(); // Esta función debe retornar el ID

                // 3. Guardas las evidencias localmente
                final evidenciasGuardadas = await guardarEvidenciasLocales(
                  denunciaId: denunciaId,
                  archivos:
                      _archivosSeleccionados, // Lista de archivos seleccionados
                  urls: _urlsEvidencias, // Lista de URLs obtenidas del servidor
                  dbHelper: DatabaseHelper(), // Tu instancia de DatabaseHelper
                );
