...
Future<String> obtenerIdentificadorUnicoDispositivo() async {
  ...
  try {
    ...
    if (Platform.isIOS) {
      ...
      identificadorUnico = iosInfo.identifierForVendor ?? 'No se pudo obtener';
    }
    // 3. Revisar si es Android
    else if (Platform.isAndroid) {
      AndroidDeviceInfo androidInfo = await deviceInfo.androidInfo;
      // El 'id' es el Android ID. Es más consistente que serialNumber.
      identificadorUnico = androidInfo.id ?? 'false';
      if (identificadorUnico == 'false') {
        identificadorUnico = androidInfo.serialNumber ?? 'No se pudo obtener';
      }
    }
  } catch (e) {
