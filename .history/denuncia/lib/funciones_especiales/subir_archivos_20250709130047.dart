...
class FileUploadSectionState extends State<FileUploadSection> {
  ...
  //mostrar evidencias por id de denuncia para el widget padre
  Future<List<Evidencia>> obtenerEvidenciasPorDenuncia(int denunciaId) async {
    try {
      final db = DatabaseHelper();
      return await db.getEvidenciasPorDenuncia(denunciaId);
    } catch (e) {
      widget.onError('Error al obtener evidencias: ${e.toString()}');
      return [];
    }
  }

  Future<String> _guardarArchivoLocalmente(File archivo) async {
  ...
  Future<bool> subirArchivos(String referenciaDenuncia) async {
    ...
    try {
      for (var archivo in _archivosSeleccionados) {
        ...
        final localPath = await _guardarArchivoLocalmente(archivo);
        pathsLocales.add(localPath);
