...
class FileUploadSectionState extends State<FileUploadSection> {
  ...
  // obtener id de la denuncia y guardarlo en una varibale global
  Future<int> _obtenerIdDenuncia(idDenuncia) async {
    ...
    if (maps.isNotEmpty) {
      // Si se encuentra la denuncia, devolver su ID
  ...
  Future<void> guardarEvidenciasLocales(int denunciaId) async {
    try {
      ...
      // Preparamos todas las evidencias
      for (var archivo in _archivosSeleccionados) {
        ...
        evidencias.add(
          Evidencia(
            ...
            url: '', // Puedes rellenarlo después si hay URL
            pathLocal: localPath,
      ...
      // Guardamos en base de datos local y copiamos archivos
      final db = await DatabaseHelper().database;
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Evidencias guardadas correctamente')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  String _determinarTipoArchivo(String path) {
    ...
    if (['pdf'].contains(extension)) return 'documento';
